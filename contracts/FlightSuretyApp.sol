...
//    mapping(bytes32 => Flight) private flights;
    ...
    mapping(address => address[]) private airlineVotes; // Keeping track of all votes for each airline
    ...
    */
    ...
    modifier requireIsAirlineFunded() {
        require(flightSuretyData.isAirlineFunded(msg.sender) == true, "Airline hasn't paid seed funding");
        _;
    }
    ...
    */
    ...
    function hasVoted(address airline) private view returns (bool) {
        address[] memory votes = airlineVotes[airline];
    ...
    returns(bool, uint256)
    {
        ...
        if (totalAirlines < 4) {
            ...
            require(hasVoted(airline) == false, "Caller has already called this function");
            airlineVotes[airline].push(msg.sender);

            uint256 requiredVotes = totalAirlines.div(2);
            uint256 castedVotes = airlineVotes[airline].length;
            ...
            if (castedVotes == requiredVotes) {
                ...
                return (true, castedVotes);
            }

            return (false, castedVotes);
        }
    }
    ...
    */
    ...
    /**
    * @dev Pay refund with credited amount to the insuree.
    *
    */
    function pay() public {
        msg.sender.transfer(flightSuretyData.pay(msg.sender));
    }
    ...
    function buy( string flight, uint256 timestamp, address airline) public payable {
        flightSuretyData.buy(flight, timestamp, airline, msg.sender, msg.value);
    }

    function checkCredit() requireIsOperational public view returns (uint256) {
...
contract FlightSuretyData {
    ...
    function registerFlight(string flightCode, uint256 timestamp, address airline) external;
    ...
    function buy(string flightCode, uint256 timestamp, address airline, address insuree, uint256 amount) external payable;

    function checkCredit( address insuree) external view returns (uint256);

    function pay(address insuree) external returns (uint256);

    function fund(address airline, uint256 amount) external payable;

    function updateFlightStatus(string flightCode, uint256 timestamp, address airline, uint8 statusCode) external;
